...
export class FirebaseStorage implements IStorage {
  ...
  async createUser(userData: Omit<User, 'id' | 'createdAt'>): Promise<User> {
    try {
      ...
      const docRef = await addDoc(collection(db, this.USERS_COLLECTION), userDoc);
...
export class MemStorage implements IStorage {
  ...
  async getUserByUsername(username: string): Promise<User | null> {
    // MemStorage doesn't support users - return null
    return null;
  }

  async createUser(userData: Omit<User, 'id' | 'createdAt'>): Promise<User> {
    // MemStorage doesn't support users - throw error
    throw new Error('MemStorage does not support user creation');
  }
}
