...
export default function BookingForm({
  ...
  const createBookingMutation = useMutation({
    ...
    onSuccess: (booking: Booking) => {
      ...
      // Invalidate all day-specific booking queries to update both time slots and day status
      queryClient.invalidateQueries({
        predicate: (query) => {
          return query.queryKey[0]?.toString().startsWith('/api/bookings/day/') || false;
        }
      });
